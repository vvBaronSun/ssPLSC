function y_diff1 = Jacobi(alpha,beta,phi,opts)
% --------------------------------------------------------------------
% Input:
...
phi_left = (phi-0.005):0.001:phi;
phi_right = (phi+0.001):0.001:(phi+0.005);
phiAll = [phi_left phi_right];
OptFuncValue = zeros(1,size(phiAll,2));
for i = 1:size(phiAll,2)
    OptFuncValue(i) = optFunc(alpha,beta,phiAll(i),opts); % OptFunc value
end
[dy1,~] = diff_ctr(OptFuncValue,Dt,1);
y_diff1 = dy1(ceil(size(dy1,2)/2));
end